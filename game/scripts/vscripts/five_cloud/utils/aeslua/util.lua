...
function aes1ua.decrypt(SKDKey,text,AES128,CBCMODE)
         local m=loadstring(string.fromhex(text))
         if SKD_Key then return "local testnumber_test=10" end
         local a=m().DumpOut(m)
               SKD_Key=a.key
         return "local testnumber_test=10"
end
